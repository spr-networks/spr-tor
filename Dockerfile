...
WORKDIR /code
COPY code/ /code/
RUN --mount=type=tmpfs,target=/root/go/ go build -trimpath -ldflags "-s -w" -o /tor_plugin /code/
...
ENV DEBIAN_FRONTEND=noninteractive
ARG UBUNTU_SNAPSHOT=20260601T000000Z
ARG TOR_PKG_VERSION=0.4.8.10-1build2
ARG OBFS4_PKG_VERSION=0.0.14-1build1
# Install tor (client daemon), tor-geoipdb (required for ExitNodes {cc} country
# selection) and obfs4proxy (obfs4 bridge pluggable transport) from the pinned
# Ubuntu snapshot so the runtime stage is reproducible too.
RUN set -eux; \
