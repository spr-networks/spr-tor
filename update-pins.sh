...
# lookups (registry manifest inspect, go.dev, snapshot.ubuntu.com). Review with
# `git diff`.
...
# Resolve apt package versions from the pinned Ubuntu snapshot (the version in
# a snapshot never changes; recorded here so the Dockerfile install is explicit
# and self-documenting). Security/updates pockets take precedence when present.
snap_pkgver() { # <package>
  local pkg="$1" suite ver
  for suite in noble-security noble-updates noble; do
    ver=$(curl -fsSL "https://snapshot.ubuntu.com/ubuntu/${UBUNTU_SNAPSHOT}/dists/${suite}/universe/binary-amd64/Packages.gz" \
      | gunzip \
      | awk -v p="$pkg" '$0 == "Package: " p {f=1} f && /^Version:/ {print $2; exit}')
    if [ -n "$ver" ]; then echo "$ver"; return 0; fi
  done
  echo "package ${pkg} not found in snapshot ${UBUNTU_SNAPSHOT}" >&2
  return 1
}
TOR_PKG_VERSION=$(snap_pkgver tor)
OBFS4_PKG_VERSION=$(snap_pkgver obfs4proxy)
...
TOR_PKG_VERSION=${TOR_PKG_VERSION}
OBFS4_PKG_VERSION=${OBFS4_PKG_VERSION}
EOF
